...
let width = dev ? 1000 + 300 : 500 + 300;
...
class Main {
	...
	createMainWindow() {
		...
		this.mainWindow = new BrowserWindow({
			...
			minWidth: width,
			minHeight: height,
			icon: path.join(__dirname, "tools.icns"),
	...
	initTray() {
		...
		const icon = nativeImage.createFromPath(path.join(__dirname, "toolsTemplate.png"));
		let tray = new Tray(icon);
		tray.setToolTip("Tools Manager");
		...
		const contextMenu = Menu.buildFromTemplate([
			...
			{ type: "separator" },
			{ label: "Quit Tools", type: "normal", click: () => this.quit() },
		]);
